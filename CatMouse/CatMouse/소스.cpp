						 ...
						 WPARAM wParam, LPARAM lParam )
{
	...
	case WM_CREATE :
		...
		hBitmap2 = (HBITMAP)LoadBitmap(((LPCREATESTRUCT)lParam)->hInstance, MAKEINTRESOURCE(IDB_BITMAP2));
	...
	case WM_PAINT :
		...
		if( push ){
			if(magic==TRUE){
				for(int i=0; i<rand()%10000; ++i){
					...
					BitBlt(hdc,mouX,mouY, 30, 36,memdc2,0,0,SRCCOPY);
					DeleteDC(memdc2);
		...
		BitBlt(hdc,catX,catY, 40, 46,memdc,0,0,SRCCOPY);
		DeleteDC(memdc);
