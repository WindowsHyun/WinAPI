...
LRESULT CALLBACK WndProc(HWND hwnd, UINT iMsg, WPARAM wParam, LPARAM  lParam)
{
	...
	case WM_PAINT :
		...
		Ellipse(hdc,isOne.left,isOne.top ,isOne.right,isOne.bottom ); //¿ÞÂÊ, Å¾ , ¿À¸¥ÂÊ, ¾Æ·¡
...
BOOL CALLBACK Dlg6_1Proc (HWND hwnd, UINT iMsg,WPARAM wParam, LPARAM lParam)
{
	...
	hdc = BeginPaint (hwnd, &ps) ;
		hBrush = CreateSolidBrush(RGB(rand()%255,rand()%255,rand()%255));
		oldBrush = (HBRUSH)SelectObject(hdc,hBrush);
		Ellipse(hdc,left,top ,right,bottom ); //¿ÞÂÊ, Å¾ , ¿À¸¥ÂÊ, ¾Æ·¡
		SelectObject(hdc,oldBrush);
		DeleteObject(hBrush);
		EndPaint (hwnd, &ps) ;
    break;
	...
	case WM_COMMAND:
		...
		case Mother_Start: // ¹öºj
			isStartMove=1;
			break;
		case Mother_Stop: // ¹öºj
			isStartMove=0;
			break;
		case EXIT: // ¹öºj
			EndDialog(hwnd,0);
