...
#define drawNemo 90
#define isEatMoney0 1000
#define isEatVAT0 100
#define isEatMoney1 4000
#define isEatVAT1 400
#define isEatMoney2 2000
#define isEatVAT2 200
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT iMsg, WPARAM wParam, LPARAM  lParam)
{
	...
	case WM_TIMER: // 초밥을 회전하기 위한 부분..!
		...
		if( 9 < WhatClick){ // 초밥을 모두 선택시 정지가 된다..!
			isGameStart = FALSE;
			KillTimer(hwnd, 1);
		}
			...
			for(int i=0; i<isMakeEatData; ++i){
				if( InEatCheck(isEatDatax[i], isEatDataxX[i], isEatDatay[i],isEatDatayY[i], mx,  my) == TRUE){
					if( 9 < WhatClick || isClick[i] ==1){
						...
						if(isEatData[i] ==  0 ){
							isWhatMoney+=isEatMoney0;
							isPlusMoney+=isEatVAT0;
						}else if(isEatData[i] ==  1 ){
							isWhatMoney+=isEatMoney1;
							isPlusMoney+=isEatVAT1;
						}else if(isEatData[i] == 2 ){
							isWhatMoney+=isEatMoney2;
							isPlusMoney+=isEatVAT2;
						}
	...
	case WM_PAINT :
		...
		for(int i=0; i <isMakeEatData; ++i){ // 전체 데이터중 현재 클릭된것을 찾아서 해당 클릭된것을 정가운데 위치에 띄워주는 부분..!
			if(1 == isClick[i]){
				...
				}else if(1 == isEatData[i]){
					Ellipse(hdc,250 ,250, 300, 300 ); //왼쪽, 탑 , 오른쪽, 아래
				}else if(2 == isEatData[i]){
			...
			if(1 == isCheckClick[i]){ // 전체데이터중 클릭이 되어있을경우 HS_DIAGCROSS 사용하여 클릭된것처럼 보이게 설정
